<template>
    <audio src="pat-and-mat-theme.webm" ref="audioEl" autoplay loop />
    <div
        v-if="!welcomeFinished"
        ref="modalEl"
        class="h-full bg-dark-400 w-full top-0 left-0 greeting-modal fixed"
    >
        <p
            ref="greetingEl"
            class="font-bold m-5 transform top-1/2 left-1/2 text-blue-300 text-4xl w-50 -translate-x-1/2 -translate-y-1/2 greeting absolute relative pointer-events-none"
            @animationiteration="updateGreeting"
        >{{ greeting }}</p>

        <button
            class="font-semibold outline-none text-white right-5 bottom-5 uppercase absolute focus:outline-none"
...
const audioEl = ref({} as HTMLAudioElement)

const startLoweringVolume = () => {
    const id = setInterval(() => {
        if (!audioEl.value || audioEl?.value?.volume < 0.01) {
            clearInterval(id)
        } else {
